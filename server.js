...
const basicAuthMiddleware = require('./src/middleware/basicAuthMiddleware.js');
...
const AIHorde = require("./src/horde/index.js");
...
const poe = require('./poe-client.js');
...
app.post('/update_config', jsonParser, async (request, response) => {
    const data = request.body
    // 读取config.mjs文件
    fs.readFile('./slaude/config.mjs', 'utf8', function (err, d) {
        if (err) {
            return console.log(err);
        }
        const newData = d.replace(/\$.*?\$/g, `$${JSON.stringify(data)}$`);
        // 将修改后的内容写入config.mjs文件
        fs.writeFile('./slaude/config.mjs', `${newData}`, function (err) {
            if (err) {
                return console.log(err);
            }
            console.log('config.mjs文件已更新');
        });
    })
})
