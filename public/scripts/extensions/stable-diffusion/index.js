...
async function sendGenerationRequest(prompt, callback) {
    const useLocal = JSON.parse(localStorage.getItem('useLocal') || 'false')
    if (useLocal) {
        await generateExtrasImage(prompt, callback);
    } else if (extension_settings.sd.horde) {
...
async function generateExtrasImage(prompt, callback) {
    ...
    const url = new URL(localStorage.getItem('local_url') || 'http://127.0.0.1:7860')
...
jQuery(async () => {
    ...
    <div class="sd_settings">
        ...
        <div class="inline-drawer-content">
            ...
            <div class="flex-container flexGap5 marginTop10 margin-bot-10px">
                ...
                <label style="margin-left:1em;" class="checkbox_label">
                    <input id="sd_local" type="checkbox"/>
                    使用自定义SD
                </label>
            </div>
            <div>
                <input id="local_url" type="text" class="text_pole" style="visibility: hidden;" placeholder="输入url，如http://localhost:7860"/>
            </div>
    ...
    var useLocal = localStorage.getItem('useLocal') || 'false'
    var local_url = $('#local_url').get(0)
    if (JSON.parse(useLocal)) {
        $('#sd_local').prop('checked', true)
        local_url.style.visibility = 'visible'
        document.querySelector('#local_url').value = localStorage.getItem('local_url') || ''
    } else {
        $('#sd_local').prop('checked', false)
        local_url.style.visibility = 'hidden'
        document.querySelector('#local_url').value = localStorage.getItem('local_url') || ''
    }

    $('#sd_local').change(function () {
        var checked = $(this).prop('checked');
        console.log(checked);
        localStorage.setItem('useLocal', checked ? 'true' : 'false')
        var local_url = $('#local_url').get(0)
        if (checked) {
            local_url.style.visibility = 'visible'
        } else {
    ...
    $('#local_url').on('blur', function () {
        localStorage.setItem('local_url', $('#local_url').val())
    })
