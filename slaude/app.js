...
const config = { ...JSON.parse(cfg.DATA.replaceAll('$', '') || '{}'), ...cfg }
    ...
    try {
        ...
        if (stream) {
            ...
            ws.on("message", (message) => {
                streamQueue = streamQueue.then(streamNextClaudeResponseChunk.bind(this, message, res, tsThread));
            });
...
function getNextChunk(text) {
    ...
    if (!text.includes(lastMessage)) {
        console.log('text check error,restore history');
    }
 ...
 */
function streamNextClaudeResponseChunk(message, res, tsThread) {
    return new Promise((resolve, reject) => {
        try {
            let data = JSON.parse(message);
            if (data.subtype === 'message_changed') {
                if (tsThread === data.message.thread_ts && config.CLAUDE_USER === data.message.user) {
 ...
 * @param {*} res The Response object for SillyTavern's request
 */
function getClaudeResponse(tsThread, message, res) {
    try {
        let data = JSON.parse(message);
        //console.log(` ${JSON.stringify(data)} \n\n`);
        try {
